...
this.makeHtml = function(text,safe) {
//
...
//
	...
	*/
	...
	/*
		...
		/g,writeAnchorTag);
	*/
	text = text.replace(/(\[((?:\[[^\]]*\]|[^\[\]])*)\]\([ \t]*()<?((https?|\/).+?)>?[ \t]*((['"])([A-Za-z0-9\-\. ]*?)\6[ \t]*)?\))/g,writeAnchorTag);
...
var writeAnchorTag = function(wholeMatch,m1,m2,m3,m4,m5,m6,m7) {
	...
	url = encodeURI(escapeCharacters(url,"*_"));
...
//
	...
	*/
	...
	/*
		...
		/g,writeImageTag);
	*/
	text = text.replace(/(!\[(.*?)\]\s?\([ \t]*()<?((https|\/)\S+?)>?[ \t]*((['"])(.*?)\6[ \t]*)?\))/g,writeImageTag);
...
var _DoItalicsAndBold = function(text) {
    ...
    text = text.replace(/(^|[^\w*_])(\*\*|__)(?=\S)([^\r]*?\S[*_]*)\2/g,
                        "$1<strong>$3</strong>");

    text = text.replace(/(^|[^\w*_])(\*|_)(?=\S)([^\r]*?\S)\2/g,
                        "$1<em>$3</em>");
